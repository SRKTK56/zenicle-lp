# ゼニくる デザイン仕様書（旧ダークテーマ・保管用）

> **2026-09-16 注記: この文書は旧ダークテーマ（`?theme=classic`）の仕様です。現行のライト基調の正本は
> `design/README.md` §1〜2（トークン対応表・共通コンポーネント指針）と `index.html` の `:root` です。**
> 新しい判断はそちらに書き、ここは classic テーマを触るときだけ参照する。
